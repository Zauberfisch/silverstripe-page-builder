	...
	static useGroupedElementPropLists(elementPropLists) {
		...
		for (let i = 0; i < count; i++) {
			...
			keys.map((key, j) => {
				...
				_clearHandlers.push(group[key].clearHandler)
				_removeHandlers.push(group[key].removeHandler)
				_moveUpHandlers.push(group[key].moveUpHandler)
				_moveDownHandlers.push(group[key].moveDownHandler)
			})
